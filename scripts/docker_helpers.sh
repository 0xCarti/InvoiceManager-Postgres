...
POSTGRES_DATABASE_URL="${DATABASE_URL:-${DATABASE_DRIVER}://${DATABASE_USER}:${DATABASE_PASSWORD}@postgres:${DATABASE_PORT}/${DATABASE_NAME}}" # Compose services resolve each other by service name (`postgres`).
